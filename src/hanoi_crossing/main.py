...
def parse_moves(lines: list[str]) -> list[Move]:
    ...
    Turn order is arbitrary — no alternation enforced.
    """
    moves: list[Move] = []
    ...
    for i, raw in enumerate(lines, start=1):
        ...
        moves.append(Move(player=player, action=action, pole_id=pole_id))
...
def print_result(result: GameResult) -> None:
    ...
    for record in result.history:
        if record.valid:
            ...
            status = f"ILLEGAL (skipped): {record.reason}"
